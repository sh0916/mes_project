...
function userInfo() {
	...
	xhr.open('GET', '/mes_project/mypage?action=userinfo', true);

	xhr.onload = function() {
		if (xhr.status >= 200 && xhr.status < 300) {
			var response = JSON.parse(xhr.responseText);

			document.querySelector('#myprofliePic div').innerHTML = '아이디 : ' + response.userId + '<br> 이름 : ' + response.name; // userId로 수정
			document.querySelector('#mycard').innerHTML = '전화번호 : ' + response.tel + '<br> 이메일 : ' + response.email;
		} else {
			// 요청이 실패했을 때 실행될 코드
...
function updateAccount() {
	...
	var xhr = new XMLHttpRequest();
	xhr.open("POST", "/mes_project/mypage?action=update", true);
...
document.querySelector("#applyVacationBtn").addEventListener("click", () => {
	...
	if (matches) {
		...
		fetch('/mes_project/mypage?action=vacation', {
			method: 'POST',
			headers: {
...
function fetchAbsentList() {
	var xhr = new XMLHttpRequest();
	xhr.open("GET", "updateabsent?action=empnolist", true); // 주소 수정
	xhr.onload = function() {
		if (xhr.status >= 200 && xhr.status < 300) {
			// 서버 응답 처리
			var response = JSON.parse(xhr.responseText);
...
function updateVacationTable(data) {
	...
	limitedData.forEach(function(absent) {
		...
		var row = `<tr>
            ...
            <td class="empno">${absent.empno}</td>
            <td>${absent.startDate}<br>${absent.endDate}</td>
            <td>${absent.reason}</td>
            <td>${statusKorean}</td>
        </tr>`;
...
function updateVacationTable2(data) {
	...
	data.forEach(function(absent) {
		...
		var row = `<tr>
            <td style="display: none;"><input type="checkbox" name="requestId" id="checkBox" value="${absent.id}"></td>
            <td class="empno">${absent.empno}</td>
            <td>${absent.startDate} ~ ${absent.endDate}</td>
            <td>${absent.reason}</td>
            <td>${statusKorean}</td>
        </tr>`;
		tableBody.innerHTML += row;
